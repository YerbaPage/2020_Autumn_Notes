...
\par 参数假设检验主要利用第一章末尾推导的分布, 要注意使用的总体方差(已知时)与样本方差. 当 $\sigma$ 已知时
\begin{enumerate}
...
\par 当样本成对收集的时候需要比较不同获得方式对观察值的影响差异是否显著而进行多次重复实验, 叫做 T 检验.

\subsection{非参数假设检验}
\par \textbf{皮尔逊$\chi^{2}$ 检验}定义检验统计量 $\chi^{2}=\sum_{i=1}^{r} \frac{\left(v_{i}-n p_{i}\right)^{2}}{n p_{i}}$
